...
oPlaylistManager = function(obj_name) {
    ...
    this.setButtons = function() {
        ...
        var gb = this.bt_sortAz_normal.GetGraphics();
            ...
            gb.DrawString(String.fromCharCode(159), gdi.Font(g_font_wd3.name, g_fsize, 1), blendColors(color_bg, color_txt, 0.5), left_padding, 1, Az_w, Az_h, lc_stringformat);
        ...
        var gb = this.bt_sortAz_hover.GetGraphics();
            gb.SetTextRenderingHint(3);
            gb.FillSolidRect(0, 0, 1, Az_h, blendColors(color_bg, color_txt, 0.35));
            gb.DrawString(String.fromCharCode(159), gdi.Font(g_font_wd3.name, g_fsize, 1), color_txt, left_padding, 1, Az_w, Az_h, lc_stringformat);
        ...
        var gb = this.bt_sortZa_normal.GetGraphics();
            gb.SetTextRenderingHint(3);
            gb.DrawString(String.fromCharCode(160), gdi.Font(g_font_wd3.name, g_fsize, 1), blendColors(color_bg, color_txt, 0.5), left_padding, 1, Az_w, Az_h, lc_stringformat);
        ...
        var gb = this.bt_sortZa_hover.GetGraphics();
            gb.SetTextRenderingHint(3);
            gb.DrawString(String.fromCharCode(160), gdi.Font(g_font_wd3.name, g_fsize, 1), color_txt, left_padding, 1, Az_w, Az_h, lc_stringformat);
        ...
        var gb = this.bt_remove_normal.GetGraphics();
            ...
            gb.DrawString(String.fromCharCode(209), gdi.Font(g_font_wd2.name, g_font_wd2.Size - g_z8, 0), blendColors(color_bg, color_txt, 0.5), 0, 0, bt_w, bt_h, cc_stringformat);
        ...
        gb = this.bt_remove_hover.GetGraphics();
            gb.SetTextRenderingHint(4);
            gb.DrawString(String.fromCharCode(209), gdi.Font(g_font_wd2.name, g_font_wd2.Size - g_z2, 0), RGB(255,0,0), 0, 0, bt_w, bt_h, cc_stringformat);
    ...
    this.refresh = function(filter, exclude_autoplaylists, exclude_active, reset_offset) {
        ...
        for(var idx = 0; idx < this.total; idx++) {
            ...
            isAutoPl = fb.IsAutoPlaylist(idx);
    ...
    this.draw = function(gr) {
        ...
        var cx, cy, cw, ch, iconw = 0, row_idx = 0, t = 0, tw = 0;
        var txt_color, icon_char, xoffset;
        ...
        if(this.woffset > 0) {
            ...
            for(var i = this.offset; i < this.playlists.length; i++) {
                ...
                if(this.inputboxID == i) {
                    ...
                    if(this.playlists[i].filter_type > 0) {
                        gr.GdiDrawText(String.fromCharCode(this.playlists[i].filter_type == 1 ? 162 : 163), gdi.Font(g_font_wd2.name, g_font_wd2.Size - zoom(14, g_dpi), 0), txt_color, cx+5+iconw+5, cy-zoom(4, g_dpi), cw-iconw-zoom(6.0, g_dpi), ch, DT_RIGHT | DT_CALCRECT | DT_BOTTOM | DT_END_ELLIPSIS | DT_NOPREFIX);
                    };
            ...
            if(cPlaylistManager.showStatusBar) {
                ...
                gr.GdiDrawText(status_txt, gdi.Font(g_fname, g_fsize - 2, 0), blendColors(this.color_txt, this.color_bg, 0.25), fx+5, fy, fw-10, fh, DT_RIGHT | DT_CALCRECT | DT_VCENTER | DT_END_ELLIPSIS | DT_NOPREFIX);
                // draw sort buttons
                        ...
                        if(this.sortZa_button.checkstate(event, x, y) == ButtonStates.hover) {
                            ...
                            for(var pl=deb; pl < this.playlists.length; pl++) {
                                if(this.playlists[pl].bt_remove.checkstate(event, x, y) == ButtonStates.hover) {
                                    if(pl == plman.ActivePlaylist) plman.ActivePlaylist = pl - 1;
    ...
    this.contextMenu = function(x, y, id) {
        ...
        case (idx==101):
            ...
            g_avoid_on_playlists_changed = true;
                plman.CreatePlaylist(total, "");
                var new_label = fb.GetPlaylistName(total);
                plman.RemovePlaylist(total);
            g_avoid_on_playlists_changed = false;
            
            fb.CreateAutoPlaylist(total, new_label, "", "", 0);
            if(id==0 && cPlaylistManager.mediaLibraryPlaylist) {
                plman.MovePlaylist(total, id + 1);
                plman.ActivePlaylist = id + 1;
                fb.ShowAutoPlaylistUI(id + 1);
                id++;
            }; else {
                plman.MovePlaylist(total, id);
                plman.ActivePlaylist = id;
        ...
        case (idx==4):
            ...
            break;
        case (idx==5):
            fb.DuplicatePlaylist(id, "Copy of " + plman.GetPlaylistName(id));
            fb.ActivePlaylist = id + 1;
            break;
        case (idx==6):
            fb.ShowAutoPlaylistUI(id);
            break;
        ...
        case (idx==200):
            ...
            fb.CreateAutoPlaylist(total, "Tracks never played", "%play_counter% MISSING", "%album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%", 0);
            if(id==0 && cPlaylistManager.mediaLibraryPlaylist) {
                fb.MovePlaylist(total, id + 1);
                fb.ActivePlaylist = id + 1;
            }; else {
                fb.MovePlaylist(total, id);
                fb.ActivePlaylist = id;  
            };
            break;
        case (idx==201):
            var total = plman.PlaylistCount;
            p.playlistManager.inputboxID = -1;
            fb.CreateAutoPlaylist(total, "Tracks played in the last 5 days", "%last_played% DURING LAST 5 DAYS", "%last_played%", 0);
            if(id==0 && cPlaylistManager.mediaLibraryPlaylist) {
                fb.MovePlaylist(total, id + 1);
                fb.ActivePlaylist = id + 1;
            }; else {
        ...
        case (idx==210):
            var total = plman.PlaylistCount;
            p.playlistManager.inputboxID = -1;
            fb.CreateAutoPlaylist(total, "Tracks unrated", "%rating% MISSING", "%album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%", 0);
            if(id==0 && cPlaylistManager.mediaLibraryPlaylist) {
                fb.MovePlaylist(total, id + 1);
                fb.ActivePlaylist = id + 1;
            }; else {
        ...
        case (idx==211):
            var total = plman.PlaylistCount;
            p.playlistManager.inputboxID = -1;
            fb.CreateAutoPlaylist(total, "Tracks rated 3 to 5", "%rating% GREATER 2", "%album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%", 0);
            if(id==0 && cPlaylistManager.mediaLibraryPlaylist) {
                fb.MovePlaylist(total, id + 1);
                fb.ActivePlaylist = id + 1;
            }; else {
        ...
        case (idx==212):
            var total = plman.PlaylistCount;
            p.playlistManager.inputboxID = -1;
            fb.CreateAutoPlaylist(total, "Tracks rated 4", "%rating% IS 4", "%album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%", 0);
            if(id==0 && cPlaylistManager.mediaLibraryPlaylist) {
                fb.MovePlaylist(total, id + 1);
                fb.ActivePlaylist = id + 1;
            }; else {
        ...
        case (idx==213):
            var total = plman.PlaylistCount;
            p.playlistManager.inputboxID = -1;
            fb.CreateAutoPlaylist(total, "Tracks rated 5", "%rating% IS 5", "%album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%", 0);
            if(id==0 && cPlaylistManager.mediaLibraryPlaylist) {
                fb.MovePlaylist(total, id + 1);
                fb.ActivePlaylist = id + 1;
            }; else {
        ...
        case (idx==214):
            var total = plman.PlaylistCount;
            p.playlistManager.inputboxID = -1;
            fb.CreateAutoPlaylist(total, "Loved Tracks", "%mood% GREATER 0", "%album artist% | $if(%album%,%date%,'9999') | %album% | %discnumber% | %tracknumber% | %title%", 0);
            if(id==0 && cPlaylistManager.mediaLibraryPlaylist) {
                fb.MovePlaylist(total, id + 1);
                fb.ActivePlaylist = id + 1;
            }; else {
