...
oHeaderBar = function() {
    ...
    this.drawColumns = function(gr) {
        var j = 0, tmp = this.x, cx = 0, cy = 0, cw = 0, sx = 0,bx = 0;
            ...
            for(var i = 0; i < 7; i++) {
                switch(i) {
                    ...
                    case 1:
                        ...
                        break;
                    case 2:
                          fields.push(new Array("null","null","null","$if2(%play_counter%,$if2(%play_count%,0))'x'","$if(%length%,%artist%,)","null","null","$if2(%genre%,'Other')","null","null","null","null","null","$if(%__bitrate_dynamic%, $if(%isplaying%,$select($add($mod(%_time_elapsed_seconds%,2),1),%__bitrate_dynamic%,%__bitrate_dynamic%)'K',$if($stricmp($left(%codec_profile%,3),'VBR'),%codec_profile%,%__bitrate%'K')),$if($stricmp($left(%codec_profile%,3),'VBR'),%codec_profile%,%__bitrate%'K'))"));
                        break;
                    case 3:
                        fields.push(new Array("0","5000","0","5000","40000","0","30000","0","0","0","10000","0","0","10000"));
                        break;
                    case 4:
